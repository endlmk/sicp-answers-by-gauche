...
(define (rand-update x)
  (let ((A 6364136223846793005)
    ...
    (modulo (+ (* A x) B) m)))
(use srfi-27)
(define (rand-update x) (random-integer (expt 2 64)))
(define random-numbers
  (cons-stream rand-init (stream-map rand-update random-numbers)))
...
(define pi (stream-map (lambda (p) (if (= p 0) 0 (sqrt (/ 6 p)))) (monte-carlo cesaro-stream 0 0)))
    
;;ex3.81
(define (random-numbers req-stream)
  (define (next req x)
    (if (eq? req 'generate) (rand-update x) req))
  (cons-stream rand-init (stream-map next req-stream (random-numbers req-stream))))
(define req1 (cons-stream 'generate req1))
(define req2 (cons-stream 'generate (cons-stream 500 req1)))
(define rs1 (random-numbers req1))
(stream-head rs1 10)
(define rs2 (random-numbers req2))
(stream-head rs2 10)

;;ex3.82
(define (test-p-stream p x1 x2 y1 y2)
    (cons-stream (p (random-in-range x1 x2) (random-in-range y1 y2)) (test-p-stream p x1 x2 y1 y2)))
(define (estimate-integral p x1 x2 y1 y2)
  (define ts (test-p-stream p x1 x2 y1 y2))
  (let ((rectarea (* (- x2 x1) (- y2 y1))))
    (scale-stream (monte-carlo ts 0 0) rectarea)))
(define (random-in-range low high)
  (use srfi-27)
  (let ((range (- high low)))
    (+ low (* (random-real) range))))
...
(/ (stream-ref (estimate-integral circle-p 2 8 4 10) 1000) (* (square 3) 1.0))
