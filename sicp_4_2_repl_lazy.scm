...
;;applyはメタ循環評価器のものとするため、先にapplyを評価する
...
;;  (if (no-operands? exps)
;;      ()
;;      (cons (evalL (first-operand exps) env)
;;	    (list-of-values (rest-operands exps) env))))
(define (eval-if exp env)
  (if (true? (actual-value (if-predicate exp) env))
      (evalL (if-consequent exp) env)
      (evalL (if-alternative exp) env)))
(define (eval-sequence exps env)
  (cond ((last-exp? exps) (evalL (first-exp exps) env))
	(else (evalL (first-exp exps) env)
...
(define (eval-definition exp env)
  (define-variable! (definition-variable exp)
    (evalL (definition-value exp) env)
    env)
  'ok)
...
(define (define-variable! var val env)
  (let ((frame (first-frame env)))
    (define (scan vars vals)
      (cond ((null? vars) (add-binidings-to-frame! var val frame))
	    ((eq? var (car vars)) (set-car! vals val))
	    (else (scan (cdr vars) (cdr vals)))))
    (scan (frame-variables frame) (frame-values frame))))
