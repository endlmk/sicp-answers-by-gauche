(define explicit-control-evaluator
  (make-machine
   ...
   '(
     ...
     (goto (reg continue))
     compound-apply
     (assign unev (op procedure-parameters) (reg porc))
     ...
     (save env)
     (save continue)
     (assigne continue (label ev-assignement-1))
     (goto (label eval-dispatch))
     ev-assignement-1
     (restore continue)
     (restore env)
     (restore unev)
     (perform (op set-variable-value!) (reg unev) (rev val) (reg env))
     (assign val (const ok))
     (goto (reg continue))
     ev-definition
